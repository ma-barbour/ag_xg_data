...
# This logs an error report and stops the script in the event of a failure

if (retry_count >= max_retries) {
        
        stop(paste("Failed at nst_data_5gp_ind after maximum retries. Error message:", error_message))
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at nst_data_5gp_oi with error message:", error_message))
...
while (retry_count < max_retries) {
        ...
        nst_data_season_ind <- tryCatch(
                ...
                nst_data_season_ind <- nst_data_season_ind |>
                        ...
                        mutate(assists = round((first_assists_60 * (toi / 60)) + (second_assists_60 * (toi / 60)))) |>
                        mutate(points = goals + assists) |>
                        mutate(goals_84 = round((goals / gp) * 84)) |>
                        mutate(points_84 = round((points / gp) * 84)) |>
                        group_by(position) |>
                        mutate(i_shots_60_rank = as.integer(rank(-shots_60, ties.method = "first"))) |>
                        mutate(i_cf_60_rank = as.integer(rank(-i_cf_60, ties.method = "first"))) |>
                        mutate(i_scf_60_rank = as.integer(rank(-i_scf_60, ties.method = "first"))) |>
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at nst_data_season_ind after maximum retries. Error message:", error_message))
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at nst_data_season_oi after maximum retries. Error message:", error_message))
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at nhl_gl_data after maximum retries. Error message:", error_message))
...
get_play_by_play_data <- readRDS("functions/get_play_by_play_data.rds")

get_on_ice_html <- readRDS("functions/get_on_ice_html.rds")
...
for(i in 1:length(new_game_ids)) {
        ...
        if (is.null(error_message) == FALSE) {
                ...
                stop(paste("Failed at refresh period data for game_id", new_game_ids[i], "with error message:", error_message))
...
xgboost_preprocessing_xg_2026 <- readRDS("models/xgboost_preprocessing_xg_2026.rds")

xg_model_v3_2026 <- readRDS("models/xg_model_v3_2026.rds") |>
        unbundle()

shot_prob_model_2026 <- readRDS("models/shot_prob_model_2026.rds") |>
        unbundle()

xg_adjustment_2026 <- readRDS("models/xg_adjustment_2026.rds")
...
xg_data <- xgboost_preprocessing_xg_2026(full_season_pbp)

xg_predictions <- predict(xg_model_v3_2026, 
                          new_data = xg_data,
                          type = "prob") |>
        select(.pred_goal)
...
full_season_xg_oi <- full_season_pbp |>
        left_join(xg_adjustment_2026, by = "shooter_id") |>
        mutate(xg_adjustment_500 = if_else(is.na(xg_adjustment_500) == TRUE, 1, xg_adjustment_500)) |>
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at skater_names after maximum retries. Error message:", error_message))
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at teams_data after maximum retries. Error message:", error_message))
...
# Add team level all-strengths xG data - full season
...
# Add team level all-strengths xG data - last 5 GP
...
# Add team level 5v5 xG data - full season

season_home_games_xg_5v5 <- full_season_xg_oi |>
...
season_away_games_xg_5v5 <- full_season_xg_oi |>
        filter(away_goalies_oi == 1) |>
        filter(home_goalies_oi == 1) |>
        filter(away_skaters_oi == 5) |>
        filter(home_skaters_oi == 5) |>
...
season_games_xg_5v5 <- season_home_games_xg_5v5 |>
        left_join(season_away_games_xg_5v5 |> select(game_id, away_xg),
                  by = "game_id")

season_home_team_xg_5v5 <- season_games_xg_5v5 |>
        group_by(home_team) |>
        summarise(home_team_xg = sum(home_xg),
                  home_opp_xg = sum(away_xg),
                  .groups = "drop") |>
...
season_away_team_xg_5v5 <- season_games_xg_5v5 |>
        group_by(away_team) |>
        summarise(away_team_xg = sum(away_xg),
                  away_opp_xg = sum(home_xg),
                  .groups = "drop") |>
...
team_xg_data <- team_xg_data |>
        left_join(season_team_xg_5v5,
                  by = "team")

# Add team level all-strengths xG data - last 5 GP

five_gp_team_xg_5v5 <- list()
teams <- season_team_xg$team
...
for(i in 1:length(teams)) {
        ...
        loop_five_gp_team_xg <- loop_five_gp_home_team_xg |>
                full_join(loop_five_gp_away_team_xg,
                          by = "team") |>
                mutate(across(where(is.numeric), ~replace_na(., 0))) |>
                mutate(xg_five_gp_5v5 = home_team_xg + away_team_xg) |>
                mutate(opp_xg_five_gp_5v5 = home_opp_xg + away_opp_xg) |>
                mutate(xg_perc_five_gp_5v5 = xg_five_gp_5v5 / (xg_five_gp_5v5 + opp_xg_five_gp_5v5))  |>
                arrange(team) |>
                select(team,
                       xg_five_gp_5v5,
                       opp_xg_five_gp_5v5,
        ...
        five_gp_team_xg_5v5[[i]] <- loop_five_gp_team_xg
        
}

five_gp_team_xg_5v5 <- five_gp_team_xg_5v5 |>
        bind_rows()

team_xg_data <- team_xg_data |>
        left_join(five_gp_team_xg_5v5,
                  by = "team")
...
while (retry_count < max_retries) {
        ...
        toi_5v5 <- tryCatch(
                ...
                {toi_5v5_data <- nst_pull(toi_5v5_url)
                
                toi_5v5_data <- toi_5v5_data |>
        ...
        if (is.null(toi_5v5) == FALSE) {break}
...
if (retry_count >= max_retries) {
        ...
        stop(paste("Failed at toi_5v5 after maximum retries. Error message:", error_message))
